...
& (Join-Path $project "idf.ps1") -IdfArguments @(
    ...
    "-D", "D2E_ALLEY_CAT=ON",
    "-D", "D2E_SHELL=ON",
    "-D", "D2E_SHELL_AUTORUN=$(if ($Headless -or $ScriptedInput) { 'ON' } else { 'OFF' })",
    "-D", "D2E_QEMU_EXIT_AFTER_RETURN=$(if ($Headless) { 'ON' } else { 'OFF' })",
...
if (-not (Select-String -LiteralPath $log -Quiet `
        -Pattern "D2E_DRIVE_READY,drive=A,type=xip,")) {
    throw "QEMU did not mount the emulated XIP A: drive. See $log"
}
if (-not (Select-String -LiteralPath $log -Quiet `
...
Write-Host "QEMU ST7789/XIP/SDSPI run passed: $log"
if ($AudioCapture) {
