...
$stringGenerated = Join-Path $outputDirectory "native_string.c"
$stringAssembly = Join-Path $outputDirectory "native_string.xtensa.s"
$portGenerated = Join-Path $outputDirectory "native_port.c"
...
& $python (Join-Path $project "tools\d2e_translate.py") `
    --hex-input --name native_string `
    (Join-Path $project "tests\fixtures\native_string.hex") $stringGenerated
if ($LASTEXITCODE -ne 0) { throw "String/REP fixture translation failed" }

& $compiler -std=c99 -O2 -Wall -Wextra -Werror `
    -I (Join-Path $project "include") -S $stringGenerated -o $stringAssembly
if ($LASTEXITCODE -ne 0) { throw "String/REP Xtensa compilation failed" }

& $python (Join-Path $project "tools\d2e_translate.py") `
    --hex-input --name native_port `
    (Join-Path $project "tests\fixtures\native_port.hex") $portGenerated
if ($LASTEXITCODE -ne 0) { throw "Port-boundary fixture translation failed" }

& $compiler -std=c99 -O2 -Wall -Wextra -Werror `
    -I (Join-Path $project "include") -S $portGenerated -o $portAssembly
if ($LASTEXITCODE -ne 0) { throw "Port-boundary Xtensa compilation failed" }

& $python (Join-Path $project "tools\d2e_translate.py") `
    --hex-input --name native_rare `
    (Join-Path $project "tests\fixtures\native_rare.hex") $rareGenerated
if ($LASTEXITCODE -ne 0) { throw "Rare-instruction fixture translation failed" }
