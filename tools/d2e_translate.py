...
def recognize_memory_pattern(instruction: Instruction) -> MemoryPattern | None:
    ...
    if not repeated:
        return None
    if operation in ("movsb", "movsw"):
        return MemoryPattern("copy", 1 if operation.endswith("b") else 2)
    if operation in ("stosb", "stosw"):
        return MemoryPattern("fill", 1 if operation.endswith("b") else 2)
    return None


def memory_pattern_statements(
    pattern: MemoryPattern,
    operands: tuple[tuple[str, OperandValue], ...],
    cached: bool,
) -> list[str]:
    if len(operands) != 2:
        raise TranslationError("memory pattern requires two operands")
    destination, source = operands
    if destination[0] != "mem" or not isinstance(destination[1], MemoryOperand):
        raise TranslationError("memory pattern destination is not memory")
    destination_segment = memory_segment_expression(destination[1])
    destination_offset = (
        "&r_di" if cached else "&cpu->regs[D2E_X86_DI]"
    )
    count = "&r_cx" if cached else "&cpu->regs[D2E_X86_CX]"
    suffix = pattern.width * 8
    if pattern.kind == "copy":
        if source[0] != "mem" or not isinstance(source[1], MemoryOperand):
            raise TranslationError("copy pattern source is not memory")
        source_segment = memory_segment_expression(source[1])
        source_offset = "&r_si" if cached else "&cpu->regs[D2E_X86_SI]"
        call = (
            f"d2e_pattern_copy{suffix}(cpu, {source_segment}, "
            f"{destination_segment}, {source_offset}, {destination_offset}, "
            f"{count});"
        )
    else:
        value = value_expression(source, suffix, cached)
        call = (
            f"d2e_pattern_fill{suffix}(cpu, {destination_segment}, "
            f"{destination_offset}, {count}, {value});"
        )
    return [
        call,
        "if (cpu->stop_reason != D2E_X86_RUNNING) { goto finish; }",
...
def string_statements(instruction: Instruction, cached: bool) -> list[str]:
    mnemonic = instruction.mnemonic
    operands = instruction.operands
    pattern = recognize_memory_pattern(instruction)
    if pattern is not None:
        return memory_pattern_statements(pattern, operands, cached)
    repeat_mode = ""
    operation = mnemonic
    for prefix in ("repne ", "repe ", "rep "):
...
def emit_generated_header(
    image_size: int, relocation_count: int, region_count: int
) -> str:
    lines = [
        "/* Generated by tools/d2e_translate.py. Do not edit. */",
        "#ifndef D2E_GENERATED_GAME_NATIVE_H",
        "#define D2E_GENERATED_GAME_NATIVE_H",
        "",
        '#include "d2e/native_runtime.h"',
        "",
        f"extern const uint8_t program_image[{image_size}];",
    ]
    if relocation_count:
        lines.append(
            f"extern const d2e_mz_relocation program_relocations[{relocation_count}];"
        )
    lines.append("")
    for index in range(region_count):
        lines.append(
            f"uint32_t d2e_generated_region_{index:03d}("
            "d2e_x86_cpu *cpu, uint32_t block_budget);"
        )
    lines.extend(["", "#endif", ""])
    return "\n".join(lines)
...
def emit_image_unit(
    ...
    lines = [
        ...
        "",
        f"const uint8_t program_image[{image_size}] = {{",
    ...
    if relocation_count:
        lines.extend(
            [
                f"const d2e_mz_relocation program_relocations[{relocation_count}] = {{",
...
def emit_region_unit(
    ...
    lines.extend(
        emit_region(
            ...
            load_segment,
            image_format,
            f"d2e_generated_region_{index:03d}",
            handoff_on_unknown=True,
            storage="",
        )
    )
...
def emit_region_dispatcher(
    partitions: list[dict[int, list[Instruction]]],
    load_segment: int,
    image_format: str,
) -> list[str]:
    lines = [
    ...
    for index, partition in enumerate(partitions):
        ...
        lines.extend(
            [
                f"        {keyword} (target <= {constant}(0x{maximum:0{width}x})) {{",
                f"            step = d2e_generated_region_{index:03d}(cpu, block_budget - executed);",
                "        }",
            ]
        )
    lines.extend(
...
def emit_source_files(
    ...
    """Emit independent C translation units for the unified EXE frontend."""
    partitions = partition_blocks(blocks)
    files = {
        "game_native.h": emit_generated_header(
            len(image), len(relocations), len(partitions)
        ),
        "game_image.c": emit_image_unit(len(image), len(relocations)),
        "game_image.inc": emit_byte_data(image),
    }
    if relocations:
        files["game_relocations.inc"] = emit_relocation_data(relocations)
    lines = [
        "/* Generated by tools/d2e_translate.py. Do not edit. */",
        '#include "game_native.h"',
        "",
    ]
    lines.extend(emit_region_dispatcher(partitions, load_segment, image_format))
    lines.extend(
        [
            "",
            "const d2e_native_program d2e_generated_program = {",
            ...
            (
                "    .format = D2E_NATIVE_IMAGE_COM,"
                if image_format == "com"
                else "    .format = D2E_NATIVE_IMAGE_MZ,"
            ),
            f"    .load_segment = UINT16_C(0x{load_segment:04x}),",
            f"    .entry_cs = UINT16_C(0x{entry_cs:04x}),",
            f"    .entry_ip = UINT16_C(0x{entry_ip:04x}),",
            ...
            "    .image_size = sizeof(program_image),",
            (
                "    .relocations = program_relocations,"
                if relocations
                else "    .relocations = NULL,"
            ),
            (
                "    .relocation_count = sizeof(program_relocations) / sizeof(program_relocations[0]),"
                if relocations
                else "    .relocation_count = 0,"
            ),
            "    .blocks = NULL,",
            "    .block_count = 0,",
            "    .region = program_region",
    ...
    files["game_native.c"] = "\n".join(lines)
    for index, partition in enumerate(partitions):
        files[f"game_region_{index:03d}.c"] = emit_region_unit(
            partition, load_segment, image_format, index
        )
    return files
